                    ...
                    <form action="<?= route_to('admin.expense.update') ?>" method="POST"
                        ...
                        <div class="row">
                            <div class="col-md-12 mb-2">
                                ...
                                <select class="form-control select2" id="type" data-placeholder="Select Expense Type" name="type">
                                    <?php foreach ([ 'billboard' => 'Billboard', 'other' => 'Other' ] as $t => $type): ?>
                                        <option value="<?= $t ?>" <?= $expense['type'] == $t ? 'selected' : '' ?>><?= $type ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                            <div class="col-md-12 mb-2" id="cntBillboard" style="display:<?=$expense['type']==='billboard'?'block':'none'?>;">
                                    ...
                                    <?php foreach ($billboards as $t => $types): ?>
                                        <optgroup label="<?= $t ?>">
                                            <?php foreach ($types as $billboard): ?>
                                                <option value="<?= $billboard['id'] ?>" <?= $expense['billboard_id'] == $billboard['id'] ? 'selected' : '' ?>>
                            ...
                            <div class="col-md-12 mb-2">
                                ...
                                <input type="number" class="form-control" id="amount" name="amount" value="<?= $expense['amount'] ?? '0.00' ?>"/>
                            </div>
                            <div class="col-md-12 mb-2">
                                <label for="addtionalInformatoin" class="form-label">Additional Information</label>
                                <textarea class="form-control" id="addtionalInformatoin" name="addtionalInformatoin"><?=$expense['addtional_info'] ?></textarea>
                            </div>
                            <div class="col-md-12 mb-2">
                                ...
                                <div class="input-group position-relative datepicker" id="expensePicker">
                                    <input autocomplete="off" data-provide="datepicker" data-date-format="yyyy-mm-dd" data-date-autoclose="true" data-date-container="#expensePicker" type="text" class="form-control" id="expenseDate" name="expenseDate" readonly value="<?= old('expenseDate', date('Y-m-d', strtotime($expense['expense_date'] ))) ?>">
